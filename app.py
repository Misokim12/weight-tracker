...
class WeightRecord(db.Model):
    ...
    journal = db.Column(db.Text, nullable=True)  # 훈련일지 내용
...
# [지도자] 특정 선수의 전체 체중 및 훈련일지 JSON API
...
# [지도자] 전체 체중 및 훈련일지 기록 엑셀(CSV) 다운로드
...
def export_csv():
    ...
    cw.writerow(['선수 이름', '아이디', '날짜', '목표 체중(kg)', '훈련 전(kg)', '훈련 후(kg)', '오늘 감량폭(kg)', '훈련일지'])
...
def record_weight():
    ...
    if journal_text is not None:
        record.journal = journal_text.strip()
        
    db.session.commit()
    flash("체중 및 훈련일지가 저장되었습니다.", "success")
    return redirect(url_for("index"))
