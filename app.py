...
class User(UserMixin, db.Model):
    ...
    name = db.Column(db.String(80), nullable=False)  # 선수/코치 실명
...
class WeightRecord(db.Model):
    ...
    weight_before = db.Column(db.Float, nullable=True)  # 훈련 전 체중
    weight_after = db.Column(db.Float, nullable=True)   # 훈련 후 체중
...
def add_athlete():
    ...
    new_athlete.set_password("123456")  # 초기 비밀번호 123456
...
def record_weight():
    ...
    # 코치가 대신 입력할 경우 target_user_id 사용, 선수는 본인 ID 사용
